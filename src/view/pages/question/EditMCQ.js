...
export default function EditMcq() {
  ...
  useEffect(async () => {
    ...
    const getQuestion = await axios.get(
      `${BACKEND_URL}/api/v1/package-question?_id=` + params.id
    );
    console.log(getQuestion.data, "<<<<this is question DAta");
    // setQuestionData(getQuestion.data.data[0]);
    const Values = getQuestion.data.data[0];
    // setIntialQuestionState({});
    setFormData({
      // package
      ...
      >
        {({
          ...
          <div className={classes.form}>
            ...
            >
              <GrayTypography>Question {formData.questionTitle}</GrayTypography>
              <DraftEditor
                placeholder="Enter question here..."
                initialValue={formData.questionTitle}
                name="questionTitle"
